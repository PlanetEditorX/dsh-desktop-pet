...
`;
    ...
    function Bubble({ snap, taskChanged, show, hovering, onWindowToggle, windowOpen }) {
      ...
      const body = hovering
        ? e('div', { className: 'dp-detail' },
            ...
            e('button', { className: 'dp-fly', onClick: () => onWindowToggle() },
              windowOpen ? '回家（关闭独立窗口）' : '出行游玩（独立窗口）'),
          )
        : e('div', null,
      ...
      const onWindowToggle = async () => {
        ...
        try {
          if (open) {
            ...
            if (r && r.ok === false) {
              setBubble({ show: true, changed: true, text: r.error && r.error.message ? r.error.message : '无法打开独立窗口' });
              if (timerRef.current) clearTimeout(timerRef.current);
              timerRef.current = setTimeout(() => setBubble((b) => ({ ...b, show: false })), 5000);
              return;
            }
          ...
          setBubble({ show: true, changed: true, text: '操作失败（RPC 不可用）' });
        }
      };
      ...
      return e('div', null,
        e('div', { className: windowOpen ? 'dp-root hidden' : 'dp-root', ref: rootRef },
          e('div', {
            ...
            e(Bubble, { snap, show: showBubble, hovering: hover, taskChanged: bubble.changed, onWindowToggle, windowOpen }),
            e(Accessory, { accessory }),
            e(Whale, { phase, dragging: dragOver, imageSrc: imgSrc, imgFailed, onImgError: () => setImgFailed(true) }),
          ),
        ),
        windowOpen
          ? e('div', { className: 'dp-dock', onClick: onWindowToggle, title: '回家（关闭独立窗口）' }, '🐋')
          : null,
      );
    }
