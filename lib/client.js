...
.dp-pet{position:relative;pointer-events:auto;cursor:grab;width:140px;height:210px;transition:transform .35s ease}
      ...
      React.useEffect(() => {
        ...
        rpc('pet.asset', { name: poseAsset }).then((r) => {
          if (alive && r && r.ok === true && r.value && r.value.base64) {
            ...
            // 显示框 140x210（2:3，与立绘同比例，contain 全身显示）→ 目标 420x630（3x 供高分屏）
            downscaleImage(src, 420, 630).then((small) => {
              if (!alive) return;
