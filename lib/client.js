...
.dp-pet{position:relative;pointer-events:auto;cursor:grab;width:168px;height:132px;transition:transform .35s ease}
...
.dp-whale-img{width:100%;height:100%;object-fit:cover;object-position:50% 0%;transition:transform .9s ease}
...
`;
    ...
    function Accessory({ accessory }) {
      ...
      if (accessory === 'clear' || accessory === 'sun') {
        ...
        inner = e('g', null,
          e('ellipse', { cx: 17, cy: 20, rx: 11, ry: 8, fill: '#94a3b8' }),
          e('ellipse', { cx: 31, cy: 16, rx: 12, ry: 9, fill: '#a8b6c8' }),
          e('ellipse', { cx: 42, cy: 21, rx: 8, ry: 6, fill: '#94a3b8' }),
          [20, 31, 42].map((x) => e('line', { key: x, x1: x, y1: 34, x2: x - 4, y2: 44, stroke: '#3b82f6', strokeWidth: 2.2, strokeLinecap: 'round' })),
        );
    ...
    function DesktopPet({ rpc }) {
      ...
      // 按姿态加载形象（host 以 base64 提供；失败回退 SVG）。姿态变化才重新拉取。
      React.useEffect(() => {
        ...
        rpc('pet.asset', { name: poseAsset }).then((r) => {
          if (alive && r && r.ok === true && r.value && r.value.base64) {
            ...
            // 显示框 168x132，cover 下源图等比缩放 → 目标约 336x264（2x 供高分屏）
            downscaleImage(src, 336, 264).then((small) => {
              if (!alive) return;
