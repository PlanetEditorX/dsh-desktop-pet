...
.dp-pet.ok:not(.dragging) .dp-whale-img{animation:dp-breathe 3.2s ease-in-out infinite}
...
.dp-pet.dragging .dp-whale-img{transform:scale(1.06)}
.dp-mouth-ov{position:absolute;bottom:6px;left:50%;transform:translateX(-50%);width:76px;height:46px;opacity:0;transition:opacity .15s;pointer-events:none}
.dp-pet.dragging .dp-mouth-ov{opacity:1}
...
`;
    ...
    // ------------------------------------------------------------------
    function Whale({ phase, dragging, imageSrc, imgFailed, onImgError }) {
      const open = dragging;
      if (imageSrc && !imgFailed) {
        return e('div', { className: 'dp-img-wrap' },
          ...
          // 拖拽时张开的嘴巴（覆盖在图下方）
          e('svg', { className: 'dp-mouth-ov', viewBox: '0 0 76 46', 'aria-hidden': true },
            e('path', { d: 'M6 12 Q38 2 70 12 Q66 40 38 42 Q10 40 6 12 Z', fill: '#7f1d1d' }),
            e('path', { d: 'M13 16 Q38 10 63 16 Q59 36 38 37 Q17 36 13 16 Z', fill: '#f87171', opacity: 0.85 }),
            e('path', { d: 'M8 12 Q38 4 68 12', fill: 'none', stroke: '#ffffff', strokeWidth: 3, opacity: 0.9 }),
          ),
        );
      }
    ...
    function Bubble({ snap, taskChanged, show, hovering, onWindowToggle, windowOpen, errText }) {
      ...
      const body = hovering
        ? e('div', { className: 'dp-detail' },
            ...
            snap && snap.weather
              ...
              : null,
            e('button', { className: 'dp-fly', onClick: () => onWindowToggle() },
              windowOpen ? '回家' : '出行'),
    ...
    function DesktopPet({ rpc }) {
      ...
      const onWindowToggle = async () => {
        const open = !!(snap && snap.windowOpen);
        try {
          if (open) {
            await rpc('pet.window.close', {});
          } else {
            const r = await rpc('pet.window.open', {});
            if (r && r.ok === false) {
              let msg = r.error && r.error.message ? r.error.message : '无法打开独立窗口';
              try {
                const d = await rpc('pet.window.debug', {});
                if (d && d.ok === true) {
                  msg += `（electron=${d.value.electronAvailable}，BrowserWindow=${d.value.hasBrowserWindow}，进程=${d.value.processType}）`;
                }
              } catch { /* ignore */ }
              showErr(msg);
              return;
      ...
      return e('div', null,
        e('div', { className: windowOpen ? 'dp-root hidden' : 'dp-root', ref: rootRef },
          e('div', {
            className: petCls,
            ...
            onDrop,
            title: '按住可拖动位置；把要清理的文件拖到身上可吃掉（移到回收站）',
          },
            e(Bubble, { snap, show: showBubble, hovering: hover, taskChanged: bubble.changed, onWindowToggle, windowOpen, errText }),
            e(Accessory, { accessory }),
            e(Whale, { phase, dragging: dragOver, imageSrc: imgSrc, imgFailed, onImgError: () => setImgFailed(true) }),
          ),
        ),
        windowOpen
          ? e('div', { className: 'dp-dock', onClick: onWindowToggle, title: '回家' }, '🐋')
          : null,
      );
    }
