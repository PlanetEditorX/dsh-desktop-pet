...
.dp-pet.dp-flew{opacity:.15;pointer-events:none;transform:scale(.6);filter:grayscale(.6)}
...
.dp-pet.ok .dp-whale{animation:dp-breathe 3.2s ease-in-out infinite}
...
.dp-img-wrap{position:relative;width:100%;height:100%;overflow:hidden;border-radius:12px}
.dp-whale-img{width:100%;height:100%;object-fit:cover;object-position:50% 10%;transition:transform .9s ease}
...
.dp-fly{margin-top:6px;padding:4px 10px;border:1px solid #93c5fd;background:#eff6ff;color:#1e40af;border-radius:8px;font-size:11px;cursor:pointer}
.dp-fly:hover{background:#dbeafe}
.dp-fly:disabled{opacity:.6;cursor:default}
...
`;
    ...
    // ------------------------------------------------------------------
    // The whale-girl: webp artwork when available, inline SVG otherwise
    // ------------------------------------------------------------------
    function Whale({ phase, imageSrc, imgFailed, onImgError }) {
      if (imageSrc && !imgFailed) {
        return e('div', { className: 'dp-img-wrap' },
          e('img', { className: 'dp-whale-img', src: imageSrc, draggable: false, alt: '鲸鱼娘', onError: () => { if (onImgError) onImgError(); } }),
        );
      }
      return e('svg', { className: 'dp-whale', viewBox: '0 0 140 110', 'aria-hidden': true },
        ...
        // 嘴：微笑
        e('path', { className: 'dp-mouth-smile', d: 'M88 72 Q96 78 104 72', fill: 'none', stroke: '#1e3a8a', strokeWidth: 2.4, strokeLinecap: 'round' }),
    ...
    function Bubble({ snap, taskChanged, show, hovering, errText, below, onDesktopToggle, desktopBusy }) {
      ...
      const desktop = snap && snap.desktop ? snap.desktop : null;
      const body = hovering
        ? e('div', { className: 'dp-detail' },
            ...
            desktop && desktop.available
              ? e('button', { className: 'dp-fly', onClick: () => onDesktopToggle(), disabled: desktopBusy },
                  desktopBusy ? '处理中…' : (desktop.running ? '收起桌面宠物' : '显示到桌面'))
              : null,
    ...
    function DesktopPet({ rpc }) {
      ...
      const [flewOut, setFlewOut] = React.useState(false);
      ...
      // 加载 webp 形象（host 以 base64 提供；失败回退 SVG）
      React.useEffect(() => {
        if (imgCache) { setImgSrc(imgCache); return; }
        let alive = true;
        rpc('pet.asset', { name: 'left' }).then((r) => {
      ...
      // 独立桌面窗口（PowerShell WPF）：桌面版才显示按钮
      const onDesktopToggle = async () => {
        const running = !!(snap && snap.desktop && snap.desktop.running);
        setDesktopBusy(true);
        try {
          const port = Number(window.location.port) || 0;
          const r = await rpc(running ? 'pet.desktop.stop' : 'pet.desktop.start', running ? {} : { port });
          if (r && r.ok === false) {
            showErr(r.error && r.error.message ? r.error.message : '操作失败');
          } else {
            showErr(running ? '已收起桌面宠物' : '已显示到桌面（可拖动，× 关闭）');
            load();
          }
        } catch {
          showErr('操作失败（RPC 不可用）');
        }
      ...
      // 主窗口屏幕矩形上报（PowerShell 窗口「拖回自动收回」的判定依据）。
      // Electron 渲染进程 window.screenX/Y + outerWidth/Height 可用。
      React.useEffect(() => {
        const report = () => {
          if (typeof window.screenX !== 'number' || typeof window.screenY !== 'number') return;
          const rect = {
            x: window.screenX,
            y: window.screenY,
          ...
          rpc('pet.window.rect', rect).catch(() => {});
        };
        report();
        const iv = setInterval(report, 20000);
        const onResize = () => report();
        window.addEventListener('resize', onResize);
        return () => { clearInterval(iv); window.removeEventListener('resize', onResize); };
      }, [rpc]);

      // 飞出状态收敛：桌面窗口关闭（running=false）时宠物回到窗口
      React.useEffect(() => {
        if (flewOut && snap && snap.desktop && !snap.desktop.running) {
          setFlewOut(false);
          showErr('已飞回窗口');
        }
      }, [snap, flewOut]);
      ...
      const onPetMouseDown = (e) => {
        ...
        const onUp = () => {
          ...
          if (ds && ds.moved) {
            ...
            // 飞出检测：宠物被拖到窗口边缘（约 16px 内）→ 自动变成独立桌面窗口
            const EDGE = 16;
            const w = root.offsetWidth || 120;
            const h = root.offsetHeight || 120;
            const atEdge = rect.left <= EDGE || rect.top <= EDGE
              || rect.left + w >= window.innerWidth - EDGE
              || rect.top + h >= window.innerHeight - EDGE;
            if (atEdge && snap && snap.desktop && snap.desktop.available) {
              const port = Number(window.location.port) || 0;
              rpc('pet.desktop.start', { port }).then((r) => {
                if (r && r.ok === true) {
                  setFlewOut(true);
                  setBubble({ show: false, changed: false });
                  showErr('已飞到桌面（拖回窗口可收回）');
                } else if (r && r.ok === false) {
                  showErr(r.error && r.error.message ? r.error.message : '无法飞到桌面');
                }
              }).catch(() => showErr('无法飞到桌面（RPC 不可用）'));
            }
            load();
          }
        };
      ...
      // 飞出状态：宠物已在独立桌面窗口，窗口内淡出（不可交互）
      const petCls = ['dp-pet', phase, flewOut ? 'dp-flew' : ''].filter(Boolean).join(' ');
      ...
      return e('div', null,
        e('div', { className: 'dp-root', ref: rootRef },
          e('div', {
            ...
            e(Bubble, { snap, show: showBubble, hovering: hover, taskChanged: bubble.changed, errText, below: bubbleBelow, onDesktopToggle, desktopBusy }),
            e(Accessory, { accessory }),
            e(Whale, { phase, imageSrc: imgSrc, imgFailed, onImgError: () => setImgFailed(true) }),
          ),
        ),
      );
    }
