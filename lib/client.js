...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
.dp-root{position:fixed;right:14px;bottom:10px;z-index:2147483000;pointer-events:none;filter:drop-shadow(0 3px 8px rgba(30,64,175,.22))}
...
`;
    ...
    function DesktopPet({ rpc }) {
      ...
      // 把宠物节点挂到 document.body 顶层，摆脱 shell.overlay 容器
      // （z-index 仅 20）被侧边栏/详情列遮挡的问题；React 只管理节点
      // 内部子树，移动后的 DOM 位置不影响后续渲染更新。
      React.useEffect(() => {
        const node = rootRef.current;
        if (node && node.parentElement && node.parentElement !== document.body) {
          document.body.appendChild(node);
        }
        // 兜底：若仍在 overlayLayer 下，把该容器 z-index 提到最高。
        let el = node && node.parentElement;
        while (el && el !== document.body) {
          if (String(el.className || '').includes('_overlayLayer')) {
      ...
      return e('div', { className: 'dp-root', ref: rootRef },
        e('div', {
          className: petCls,
