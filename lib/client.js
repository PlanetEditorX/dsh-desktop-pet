...
`;
    ...
    function taskText(task) {
      ...
      const name = task.title || [task.provider, task.model].filter(Boolean).join(' / ') || '未知任务';
      if (task.kind === 'request') return `正在执行任务 · ${name}`;
      if (task.kind === 'reply') return `${name} 刚刚回复 · +${fmtTokens(task.tokens)} tokens`;
      return `正在执行任务 · ${name}`;
    }

    function Bubble({ snap, taskChanged, show, hovering, errText }) {
      ...
      return e('div', null,
        e('div', { className: 'dp-root', ref: rootRef },
          e('div', {
            ...
            e(Bubble, { snap, show: showBubble, hovering: hover, taskChanged: bubble.changed, errText }),
