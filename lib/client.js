...
.dp-pet.ok .dp-whale-img{animation:dp-breathe 3.2s ease-in-out infinite}
.dp-pet.hungry .dp-whale-img{animation:dp-slouch 3.4s ease-in-out infinite}
.dp-pet.collapsed .dp-img-wrap{transform:translateY(6px) scale(.94);transition:transform 1.1s ease-in-out}
.dp-pet.collapsed .dp-whale-img{animation:none}
.dp-root.hidden{opacity:0;pointer-events:none;transition:opacity .3s ease}
/* The pet is a desktop overlay. On narrow layouts its speech bubble covers
   the session-log/subagent and permission/model status controls. Hide it by
   viewport as well as UA so mobile emulation and tablets are safe too. */
@media (max-width:600px){.dp-root{display:none!important}}
...
`;
    ...
    const POSE_ASSET = {
      ...
      collapsed: 'whale-maid-collapsed',
    };
    ...
    function DesktopPet({ rpc }) {
      ...
      // 移动端（手机/平板 UA 或触摸设备）不显示宠物（可在设置中关闭）。
      // Some phones use a desktop UA and a 980px layout viewport, so UA and
      // viewport-width checks alone are insufficient; touch is the reliable
      // signal for this desktop-only overlay.
