...
export function startDesktopPet({ port }) {
  if (!isDesktopMode()) return { ok: false, reason: '独立桌面窗口仅在桌面版（DeepSeek Harness.exe）可用' };
  if (desktopPetProc && desktopPetProc.exitCode === null) return { ok: true, already: true };
...
export function apply(ctx) {
  ...
  // 用 ctx.inject 回调可选接入：webServer 就绪后才注册，缺席时不阻塞插件启动。
